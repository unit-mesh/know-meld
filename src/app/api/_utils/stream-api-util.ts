...
export function convertToStreamCompletion(response: Response): ReadableStream<string> {
    ...
    if (data) {
      ...
      for (const line of lines) {
        ...
        if (message === "[DONE]") {
          ...
          try {
            const parsed = JSON.parse(message); // 可能会抛出错误，如果数据不完整
            if (parsed.answer) {
              controller.enqueue(parsed.answer);
            } else if (parsed.event == "tts_message_end" || parsed.event == "message_end") {
              controller.close();
            }
          } catch (error) {
...
export function handleSuccessResponse(response: Response, isWorkflow = false): NextResponse {
  ...
  let stream;
  try {
    stream = convertToStreamCompletion(response);
  } catch (e) {
    console.log(e);
  }
